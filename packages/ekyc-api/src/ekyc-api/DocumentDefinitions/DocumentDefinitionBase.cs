    ...
    public abstract class DocumentDefinitionBase : IDocumentDefinition
    {
        ...
        public virtual async Task<Dictionary<string, string>> GetFieldData(string S3Key, DocumentTypes docType)
        {
            ...
            if (Globals.UseFieldCoordinatesExtractionMethod)
                return await GetFieldDataByCoordinates(S3Key, DocumentType);

            // Use the new Textract AnalyzeID API

            List<Document> documentPages = new List<Document>();

            documentPages.Add(new Document()
                { S3Object = new S3Object() { Bucket = Globals.StorageBucket, Name = S3Key } });
            ...
            {
                returnVal[result.Type.Text] = result.ValueDetection.Text;
            }
            ...
            DocumentTypes docType)
        {
            ...
            returnVal = await this.PostProcessFieldData(returnVal);
